...
# # Create random prediction tensor with shape [batch_size, num_classes, num_points]
# pred = torch.randn(batch_size, num_classes, num_points)

# # Create target tensor with shape [batch_size, num_points] containing 0s and 1s
# gold = torch.randint(0, num_classes, (batch_size, num_points))

# print(f"Prediction shape: {pred.shape}")  # Expected: [batch_size * num_points, num_classes]
# print(f"Target shape: {gold.shape}")      # Expected: [batch_size * num_points]

# # Reshape pred to [batch_size * num_points, num_classes]
# pred = pred.permute(0, 2, 1).contiguous().view(-1, num_classes)

# # Reshape gold to [batch_size * num_points]
# gold = gold.view(-1)
